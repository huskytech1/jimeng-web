...
本技能用于通过网页模拟的方式操作即梦页面完成出图：切到图片生成、选择模型、设置比例、输入提示词、等待结果、下载最新图片。当前版本已彻底移除 CLI 外壳，只保留网页模拟实现。
...
6. 在任何模式切换、比例设置和输入提示词之前，先记录“生成前最底部旧卡片”的基线签名。
7. 如果页面长时间停留在加载态但结果已在后端生成，主动刷新页面后重新定位当前任务并下载最新结果。
...
| `Generation timeout` | 页面可能卡在旧状态；先重试一次，网页自动化会自动刷新同步结果 |
| `No images were generated` | 当前任务未产出新图；检查额度、排队状态或缩短 prompt 后重试 |
| 下载到旧图/错图 | 这通常是页面旧状态导致；当前流程会优先等待最底部新增任务卡片并按最新图下载 |
